...
const SearchResults: NextPage<searchResultsPropsType> = ({ cloudData }) => {
  let { applicationTypeFilter, cloudProviderFilter, textFilter } =
    useContext(CloudPageContext);
...
function getCardListData(
  ...
  if (cloudProviderFilter != "")
    dt = cloudData.filter(
      (el) =>
        el.title.toLowerCase() == cloudProviderFilter.toLowerCase() ||
        el.url.toLowerCase().includes(cloudProviderFilter.toLowerCase())
    );
  else if (applicationTypeFilter != "")
    dt = cloudData.filter((el) =>
      el.description
        ?.toLowerCase()
        .includes(applicationTypeFilter.toLowerCase())
    );
  else if (textFilter != "")
    dt = cloudData.filter(
      (el) =>
        el.description?.toLowerCase().includes(textFilter.toLowerCase()) ||
