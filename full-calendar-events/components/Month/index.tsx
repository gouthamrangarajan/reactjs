                ...
                (<motion.tr key={`tr_${index}_${currMonthIndex}`}
                    ...
                    (
                        <Td key={`td_${index}_${index1}`} date={new Date(currYear, currMonthIndex, idt.date)}
                            ...
                            {index == 0 && <span className="select-none">{getDayName(idt.date, currMonthIndex, currYear)}</span>}
                            <h5 className={`${idt.ind != 'curr' ? "text-gray-400" : ""} px-2 py-1
                                ...
                                <>
                                    {idt.date}&nbsp;
                                    {idt.date == 1 && index == 0 ? getMonthNameShort(currMonthIndex)
                                        : idt.date == 1 ? getMonthNameShort(currMonthIndex + 1) : <></>}
                                </>
                            </h5>
