...
import {
  slideDownAnimationVariant,
  userTypeDataGridColumnHelper as columnHelper,
} from "../../util/helper";
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <motion.table className="table-auto w-full" layout>
          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <tr key={headerGroup.id}>
              {headerGroup.headers.map((header) => (
                <Th key={header.id} header={header} table={table}></Th>
              ))}
            </tr>
          ))}
        </motion.thead>
                ...
                {row.getVisibleCells().map((cell, index) => (
                  ...
                  >
                    <div className="w-full h-full flex items-center">
                      {index == 0 && (
                        <button
                          className="p-1 outline-none appearance-none focus:ring-1
            ...
            [1, 2, 3].map((trEl) => (
              <tr key={trEl}>
                {[1, 2, 3, 4, 5].map((el) => (
                  <td
                    key={el}
                    className="py-2 px-4 border-collapse border border-slate-200 w-1/5"
                  >
        ...
        <TFoot
          setShowColumnChooser={setShowColumnChooser}
